...
The rulebook distinguishes four contexts that must not be conflated:

* ``regulated_gi``: a protected-origin claim. It requires an explicit legal
  allowed-grape rule and fails closed when that rule is absent.
* ``regional_style``: a non-legal simulator plausibility guard. It may use the
  legacy ``primary_grapes`` field, but it never certifies GI eligibility.
* ``country_wine``: a country-origin wine supported by cultivation or market
  evidence for each grape.
* ``experimental``: an explicit exception path for trials and tiny plantings;
  it never creates a protected-origin claim.

This separation prevents the simulator from turning weak geographic hints into
legal facts while still allowing plausible regional and experimental gameplay.
"""
...
class RegionRule:
    ...
    @property
    def regional_style_rule_known(self) -> bool:
        return bool(self.primary_grapes)
...
class RegionGrapeRulebook:
    """Resolve geographic rules and prevent impossible origin/grape claims."""
    ...
    def _load(self) -> None:
        ...
        for country_row in doc.get("regions", []):
            ...
            for region in country_row.get("wine_regions", []):
                ...
                region_primary = tuple(
                    str(g) for g in region.get("primary_grapes", []) if str(g).strip()
                )
                ...
                for sub in region.get("sub_regions", []):
                    ...
                    sub_primary = tuple(
                        str(g)
                        for g in sub.get("primary_grapes", region_primary)
                        if str(g).strip()
                    )
                    sub_rule = RegionRule(
                    ...
                    for commune in sub.get("communes", []):
                        ...
                        allowed = tuple(
                            str(g)
                            for g in commune.get("allowed_grapes", [])
                            if str(g).strip()
                        )
                        ...
                        rule = RegionRule(
                            ...
                            max_yield_hl_ha=_positive(commune.get("max_yield_hl_ha")),
                            min_alcohol_pct=_positive(commune.get("min_alcohol")),
    ...
    def _grapes_fit(self, canonical: tuple[str, ...], permitted: Sequence[str]) -> list[str]:
        return [
            grape
            for grape in canonical
            if not any(self.same_grape(grape, allowed) for allowed in permitted)
        ]
    ...
    def commercial_evidence(
        ...
        for obs in self.catalog.commercial_observations:
            if self._norm(obs.country) == self._norm(country) and self._norm(obs.variety) in aliases:
    ...
    def evaluate(
        ...
        *,
        ...
        allowed_scopes = {"regulated_gi", "regional_style", "country_wine", "experimental"}
        ...
        if scope in {"regulated_gi", "regional_style"}:
            rule = self.resolve(
                country=country,
            ...
            if rule is None:
                return OriginDecision(
                    eligible=False,
                    status="unresolved_appellation" if scope == "regulated_gi" else "unresolved_region",
                    label_scope=scope,
                    canonical_grapes=canonical,
                    issues=("The requested origin does not resolve to a geographic rule; automatic origin generation is blocked.",),
                )
            ...
            if scope == "regulated_gi":
                if not rule.legal_grape_rule_known:
                    return OriginDecision(
                        ...
                        canonical_grapes=canonical,
                        rule_id=rule.id,
                        issues=("This origin lacks an explicit allowed-grape rule in the current registry. The simulator must not assume primary_grapes is a legal list.",),
                    )
                forbidden = self._grapes_fit(canonical, rule.allowed_grapes)
                if forbidden:
                    return OriginDecision(
                        eligible=False,
                ...
                return OriginDecision(
                    ...
                    canonical_grapes=canonical,
                    rule_id=rule.id,
                    warnings=("Experimental production flag is set; legal process rules beyond grape eligibility still require validation.",) if experimental else (),
                    evidence=tuple(f"allowed_grape:{g}" for g in rule.allowed_grapes),
                )

            if not rule.regional_style_rule_known:
                return OriginDecision(
                    eligible=False,
                    status="regional_style_rule_unverified",
                    label_scope=scope,
                    canonical_grapes=canonical,
                    rule_id=rule.id,
                    issues=("This region has no primary-grape evidence in the current catalog.",),
                )
            forbidden = self._grapes_fit(canonical, rule.primary_grapes)
            if forbidden:
                return OriginDecision(
                    eligible=False,
                    status="grape_not_plausible_for_region",
                    label_scope=scope,
                    canonical_grapes=canonical,
                    rule_id=rule.id,
                    issues=tuple(
                        f"{grape} is not in the conservative primary-grape set for {rule.commune or rule.sub_region or rule.major_region}"
                        for grape in forbidden
                    ),
                )
            return OriginDecision(
                eligible=True,
                status="regional_style_supported",
                label_scope=scope,
                canonical_grapes=canonical,
                rule_id=rule.id,
                warnings=("Regional-style eligibility is a simulator plausibility rule. It is not a legal GI/appellation certification.",),
                evidence=tuple(f"primary_grape:{g}" for g in rule.primary_grapes),
            )

        evidence: list[str] = []
        missing: list[str] = []
        for grape in canonical:
            ok, grape_evidence = self.commercial_evidence(grape, country, vintage_year=vintage_year)
            if ok:
        ...
        return OriginDecision(
            eligible=True,
            status="experimental_origin" if missing or scope == "experimental" else "country_origin_supported",
            label_scope=scope,
            canonical_grapes=canonical,
            warnings=warnings,
            evidence=tuple(sorted(set(evidence))),
    ...
    def stats(self) -> dict[str, int]:
        ...
        regional = [rule for rule in self.rules if rule.regional_style_rule_known]
        return {
            "region_rules": len(self.rules),
            "strict_allowed_grape_rules": len(strict),
            "regional_style_grape_rules": len(regional),
