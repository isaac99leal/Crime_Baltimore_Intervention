...
class MoreyLegalEnrichmentTests(unittest.TestCase):
    @classmethod
    ...
    def test_existing_four_ids_are_enriched_not_duplicated(self) -> None:
        ...
        expected = {
            "fr:morey-saint-denis:white-standard",
            "fr:morey-saint-denis:red-standard",
            "fr:morey-saint-denis:white-premier-cru",
            "fr:morey-saint-denis:red-premier-cru",
        }
        for spec_id in expected:
            self.assertEqual(ids.count(spec_id), 1)
        for suffix in (
            "white-standard",
            "red-standard",
            "white-premier-cru",
            ...
            self.assertIn(
                f"fr:morey-saint-denis:{suffix}:masa-2011-enrichment",
                self.registry.applied_amendment_ids,
            )

    def test_exact_color_and_level_maturity_matrix(self) -> None:
        expected = {
    ...
    def test_exact_elevage_and_release_calendar_applies_to_all_four_paths(self) -> None:
        for variant in (
            "white standard",
            "red standard",
            "white premier cru",
    ...
    def test_monts_luisants_aligote_exception_is_not_generalized(self) -> None:
        spec = self.spec("white premier cru")
        ...
        decision = self.registry.evaluate_blend(spec, "Aligoté")
        self.assertFalse(decision.eligible)
        self.assertEqual(decision.status, "grape_not_permitted_for_appellation")

    def test_red_accessory_co_plantings_are_not_generic_cellar_blends(self) -> None:
        spec = self.spec("red premier cru")
    ...
    def test_existing_twenty_premier_cru_climats_remain_intact(self) -> None:
        catalog = WorldWineKnowledgeCatalog()
        climats = {
            site.name
            for site in catalog.named_sites
            if site.parent == "Morey-Saint-Denis"
            and site.site_type == "climat"
            and site.classification == "Premier Cru"
        }
        self.assertEqual(len(climats), 20)
        self.assertIn("Monts Luisants", climats)
        self.assertIn("Les Ruchots", climats)
        self.assertIn("Clos des Ormes", climats)


class MoreyVineyardLawTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
    ...
    def test_complete_white_and_red_paths_pass(self) -> None:
        for style in ("white", "red"):
            ...
            self.assertIn("source:morey_masa_2011_current", assessment.evidence)
    ...
    def test_white_guyot_simple_remains_eight_buds(self) -> None:
    ...
    def test_dead_vine_threshold_uses_national_proportional_remedy(self) -> None:
        ...
        self.assertIn("threshold-source:morey_masa_2011_current", above.evidence)
